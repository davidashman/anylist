...
class Item {
   ...
   */
	...
	/**
   * Add this item to a favorites list.
   * @param {string} listId favorites list identifier
   * @return {Promise}
   */
	async addToFavorites(listId) {
		this.listId = listId;
		...
		const ops = new this._protobuf.PBListOperationList();
		...
		await this._client.post('data/starter-lists/update', {
			body: form,
		});
	}
