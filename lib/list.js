...
class List {
   ...
   * Set `isIngredient=true` to add a recipe ingredient;
   * in that case, `item` should be a plain object matching
   * the ListItem protobuf structure (including ingredients).
   * @param {Item|object} item to add
   * @param {object} [options]
   * @param {boolean} [options.isFavorite=false]
   * @param {boolean} [options.isIngredient=false]
   * @return {Promise<Item>} saved item (only when not isIngredient)
   */
	async addItem(item, options = {}) {
		if (typeof options === 'boolean') {
			options = {isFavorite: options};
		}

		const {isFavorite = false, isIngredient = false} = options;
		...
		if (isIngredient) {
			...
			listItem = new this.protobuf.ListItem(item);
		} else {
